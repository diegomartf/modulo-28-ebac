...
def main():
    ...
    if (data_file_1 is not None):
        ...
        bank_raw_target_perc = bank_raw_target_perc.sort_index().reset_index().rename(columns={'index': 'y'})

        try:
            bank_target_perc = bank.y.value_counts(normalize=True).to_frame(name='percentage') * 100
            bank_target_perc = bank_target_perc.sort_index().reset_index().rename(columns={'index': 'y'})
